...
def commands(message):
    ...
    bot.reply_to(message, "Привет! Напиши //start.")
...
def find_anekdot(message):
    '''Функция получает на входе команду /findan, /findvid или /findrandom, после чего вым присылают сообщение, на которое вам надо ответить, ващ ответ перенаправится \
        в другие функции бота. Имеется глобальная переменная k, котороя принимает значение в зависимости от того, что вы ищите. Данная переменая влияет на то, что будет выведено \
            в функции otvet'''
    global k
...
def username(message):
    '''Данная функция запоминает ваше имя, которое записывается в глобальную переменную name (по дефолту она равна-Пользователь). Она используется в сообщениях бота, чтобы обращться к вам по имени.'''
    global name
    name = message.text
    bot.send_message(message.from_user.id, 'Приятно познакомиться, ' + name + '!')
    bot.send_message(message.from_user.id, 'Напиши /help, чтобы узнать что может этот бот')
...
def otvet(call):
    '''Функция получает на входе ваш ответ (да или нет),если он отрицательный, вас возращает к предыдущей функции, если же он положительный, с помощью библиотеки selenium \
       происходит парсинг с сайтов(зависимые от вашего выбора, на это влияет переменная k, при k=0 мы ищем анекдоты, при k=1 мы ищем видео), выводит информацию в телеграм '''
    if call.data == "yes":
        if k == 0:
